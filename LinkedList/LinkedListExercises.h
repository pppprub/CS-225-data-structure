...
void LinkedList<T>::insertOrdered(const T& newData) {
  ...
  Node* newnode = new Node(newData); //create a node in the heap mem
  ...
  if(empty()){
    pushFront(newnode->data);
      ...
      bool inserted = false;
      while(!inserted){
        //the headpointer corresponds to the smallest value(after sorted), so we need to check if the inserted value less than the current value(start from the smalles value)
        //if thats true, we go the the if loop, otherwise we move the current space to the next one
        if(newnode->data <= currnode-> data){
          //update prev and next pointers for the inserted node 
          newnode->prev = currnode->prev;
          newnode->next = currnode;
          //update the prev pointer of the current node 
          currnode->prev = newnode;
          //move the current position to the node in previous 
          //and update the 'next' pointer of the previous node 
          currnode = newnode->prev;
          currnode->next = newnode;
        // change the bool to true and end the loop
         size_++;
        inserted = true;
        }
       // otherwise we move the current space to the next one
