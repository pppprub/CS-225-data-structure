...
/*if (StringVec == 'cat'){
  ...
  {
     std::string index = word;
     
     
     if( wordcount_map.find( index ) == wordcount_map.end()  )
     {
        // first time, index is not in wordcount_map before
        wordcount_map[ index ] = 1;
     }
     else
     {
        // not first time, index is in wordcount_map already
       wordcount_map[ index ] += 1;
     }
...
int lookupWithFallback(const StringIntMap& wordcount_map, const std::string& key, int fallbackVal) {
  ...
  // =================================================
int value = fallbackVal;

if(wordcount_map.find(key) != wordcount_map.end())
{
  value = wordcount_map.find(key)->second;//wordcount_map[key]
};
